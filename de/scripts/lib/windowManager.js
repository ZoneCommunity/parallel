...
function createWindow(
  ...
  win.id = `${title}-${makeID(5)}`;
  win.style.width = width;
  win.style.height = height;

  if (lastFocused) {
    let lastFocusedWin = document.getElementById(lastFocused);
    if (lastFocusedWin) lastFocusedWin.classList.remove("focused");
  }
...
function closeWindow(win) {
  ...
  openWindows.splice(
    openWindows.findIndex((win) => win.id === id),
    1
  );
  updateTaskbar();
